...
import { createBookingSchema, updateBookingDetailsSchema, submitReceiptSchema } from "./booking.schemas.js";
...
async function autoCancelIfExpired(booking) {
    if (!booking)
        return booking;
    if (booking.status !== "BOOKED" || booking.paymentStatus !== "PENDING")
        return booking;
    if (booking.paymentMessage)
        return booking;
    const now = new Date();
...
export async function createBooking(req, res) {
    ...
    const { fullName, email, purpose, address, nationalId, roomTypeId, checkIn, checkOut, phone, paymentMessage, userId } = result.data;
...
export async function getBookingLookup(req, res) {
    const { id } = req.params;
...
export async function submitReceiptMessage(req, res) {
    const { id } = req.params;
