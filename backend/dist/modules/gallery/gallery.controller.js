...
const isCloudinaryConfigured = !!(cloudinaryUrl || (cloudinaryCloudName && cloudinaryApiKey && cloudinaryApiSecret));
...
function uploadBufferToCloudinary(buffer) {
    return new Promise((resolve, reject) => {
        const uploadStream = cloudinary.uploader.upload_stream({ folder: "dream_hotel_gallery" }, (error, result) => {
            if (error)
                return reject(error);
            if (!result)
...
export async function getGalleryImages(_req, res) {
