...
export default function BookingDetailsSummary({ booking, onStartEdit }: BookingDetailsSummaryProps) {
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 sm:gap-y-4 gap-x-3 sm:gap-x-6 text-xs font-mono">
      ...
      <div className="border-t border-slate-100 pt-3.5 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 text-xs font-mono">
      ...
      {booking.status === "BOOKED" && (
        <div className="flex flex-col sm:flex-row gap-2 pt-1 border-t border-slate-100">
          <button
            ...
            className="w-full sm:w-auto px-3.5 py-2 rounded-xl border border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-indigo-600 hover:text-indigo-700 font-sans font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
          >
