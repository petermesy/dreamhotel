...
function ReservationsTab(props: BackOfficeProps) {
  ...
  return (
    <>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col gap-3 md:flex-row md:items-center font-mono text-xs">
          ...
          {props.reservations.length === 0 ? (
            ...
            <div key={booking.id} className="border-b border-slate-100 p-4 space-y-3 last:border-b-0">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="font-bold text-slate-900">ETB {booking.totalPrice.toLocaleString()}</div>
                <div className="flex flex-wrap justify-end gap-1.5">
                  {booking.status === "BOOKED" && <button onClick={() => props.onStatusChange(booking.id, "CHECKED_IN")} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-2 py-1 rounded text-[10px] transition-colors cursor-pointer">Check In</button>}
                  {booking.status === "CHECKED_IN" && <button onClick={() => props.onStatusChange(booking.id, "CHECKED_OUT")} className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-2 py-1 rounded text-[10px] transition-colors cursor-pointer">Check Out</button>}
                  {["BOOKED", "CHECKED_IN"].includes(booking.status) && <button onClick={() => props.onCancelReservation(booking.id)} className="border border-rose-200 hover:border-rose-300 bg-rose-50 text-rose-700 hover:bg-rose-100 font-bold px-2 py-1 rounded text-[10px] transition-all cursor-pointer">Cancel</button>}
                  {props.isOwner && <button onClick={() => props.onDeleteReservation(booking.id)} className="bg-rose-600 hover:bg-rose-700 text-white font-bold p-1 rounded transition-colors cursor-pointer" title="Permanently Delete Booking (Owner Exclusive)"><Trash2 className="w-3.5 h-3.5" /></button>}
                </div>
              </div>
            </div>
          ))}
